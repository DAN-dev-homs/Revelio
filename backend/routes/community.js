...
router.post('/posts/:id/like', auth, (req, res) => {
  ...
  const post = req.db.prepare('SELECT id, user_id FROM posts WHERE id = ?').get(postId);
  ...
  if (existing) {
    req.db.prepare('DELETE FROM post_likes WHERE post_id = ? AND user_id = ?').run(postId, req.user.id);
    req.db.prepare('UPDATE posts SET likes_count = MAX(0, likes_count - 1) WHERE id = ?').run(postId);
    res.json({ liked: false });
  } else {
    req.db.prepare('INSERT INTO post_likes (post_id, user_id) VALUES (?, ?)').run(postId, req.user.id);
    req.db.prepare('UPDATE posts SET likes_count = likes_count + 1 WHERE id = ?').run(postId);

    if (post.user_id !== req.user.id) {
      const liker = req.db.prepare('SELECT name FROM users WHERE id = ?').get(req.user.id);
      req.db.prepare(
...
router.post('/posts/:id/comments', auth, (req, res) => {
  ...
  const post = req.db.prepare('SELECT id, user_id FROM posts WHERE id = ?').get(req.params.id);
  if (!post) return res.status(404).json({ error: 'Post not found' });
