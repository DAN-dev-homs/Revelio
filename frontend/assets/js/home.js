...
const HomePage = (() => {
  ...
  async function render(container) {
    ...
    const readingResponse = await api.getReadingList().catch(() => []);
    const reading = Array.isArray(readingResponse) ? readingResponse : [];
    const notifsData = (await api.getNotifications().catch(() => null)) || { unreadCount: 0 };
    const allBooksResponse = await api.getBooks().catch(() => []);
    const allBooks = Array.isArray(allBooksResponse) ? allBooksResponse : [];
