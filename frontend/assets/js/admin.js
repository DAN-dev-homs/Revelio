  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card">
          <div class="icon">📚</div>
          <div class="label">Total enseignements</div>
        ...
        <div class="stat-card">
          ...
          <div class="label">Enseignements sauvegardés</div>
    ...
    // Afficher les enseignements populaires
    ...
    return `
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
        ...
        <div style="flex:1;">
          ...
          <div style="font-size:12px;color:var(--muted);">${reader.books_read} enseignements · ${Math.round(reader.total_progress || 0)}% progression</div>
        </div>
...
async function loadBooks() {
  ...
  try {
    ...
    tbody.innerHTML = books.length === 0
      ? '<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:32px;">Aucun enseignement</td></tr>'
      : books.map(b => `
        <tr>
...
function openBookModal() {
  ...
  document.getElementById('book-modal-title').textContent = 'Ajouter un enseignement';
...
async function openEditBookModal(id) {
  try {
    ...
    document.getElementById('book-modal-title').textContent = 'Modifier l\'enseignement';
    ...
    alert("Impossible de charger les détails de l'enseignement : " + e.message);
  }
}
